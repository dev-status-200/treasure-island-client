...
const Services = ({ sessionData, parts, servicesData }) => {
  ...
  return (
    <div>
      <ServiceLayout parts={parts} servicesData={servicesData} />
    </div>
  );
};
...
export async function getServerSideProps({ req, res }) {
  ...
  const requestOne = await axios
    .get('https://treasure-island-server.herokuapp.com/getUser', {
      headers: {
  ...
  const requestTwo = await axios
    .get(process.env.NEXT_PUBLIC_TI_GET_PARTS)
    .then((x) => x.data);

  const requestThree = await axios
    .get(process.env.NEXT_PUBLIC_TI_GET_SERVICES)
    .then((x) => x.data);
