...
const MechanicsLayout = ({ mechanics }) => {
  ...
  const getMail = async () => {
    let res = await axios
      .get(process.env.NEXT_PUBLIC_TI_MECHANIC_EMAILS)
      .then((x) => x.data);
    console.log(res);
    setMailList(res);
  };
  const getMailEdit = async (ids) => {
    let res = await axios
      .get(process.env.NEXT_PUBLIC_TI_MECHANIC_EMAILS_EDIT, {
        headers: {
          id: `${ids}`,
        },
      })
  ...
  async function uploadImage() {
    ...
    if (change == true) {
      if (image != '') {
        ...
        value = await fetch(
          `https://api.cloudinary.com/v1_1/abdullah7c/image/upload`,
          {
            method: 'post',
            body: data,
          }
        ...
        value =
          'https://res.cloudinary.com/abdullah7c/image/upload/v1643040095/images_djois2.png';
      }
    } else {
      value = image;
    }
  ...
  const addAgent = async (e) => {
    ...
    password.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,20}$/) == null
      ? (passWarning = true)
      : (passWarning = false);
    ...
    if (
      ...
      setMailWarn(false);
      await axios
        .post(process.env.NEXT_PUBLIC_TI_ADD_MECHANICS, {
          f_name: f_name,
          l_name: l_name,
          password: password,
          gender: gender,
  ...
  const editMechanic = async (e) => {
    ...
    if (
      ...
      axios
        .put(process.env.NEXT_PUBLIC_TI_EDIT_MECHANICS, {
          id: id,
          f_name: f_name,
          l_name: l_name,
          password: password,
  ...
  const deleteUser = () => {
    setLoad(true);
    axios
      .post(process.env.NEXT_PUBLIC_TI_DELETE_MECHANICS, { id: id })
      .then((x) => {
        if (x.data[0] == '1') {
          let tempState = [...MechanicList];
    ...
    <div className="mechanic-styles">
            ...
            <Col>
              <span>
                <button
                  ...
                  style={{ float: 'right' }}
                  onClick={() => setNumSearch(!numSearch)}
                >
                  <RiSortDesc />
                </button>
              </span>
                            ...
                            <tr key={index} className="">
                              <td>
                                <Row>
                                  <Col md={2}>
                                    <img
                                      src={mech.profile_pic}
                                      className="image"
                                    />
                                  </Col>
                                  <Col md={10}>
                                    <div className="name pt-2">
                                      <span>{mech.f_name}</span>{' '}
                                      <span>{mech.l_name}</span>
                                    </div>
                                    <div
                                      style={{
                              ...
                              <td className="phone py-3">
                                ...
                                />
                                <AiFillEdit
                                  className="yellow icon-trans"
                                  onClick={() => {
                                    editFields(mech);
                                  }}
                                />
                                <AiFillDelete
                                  className="red icon-trans"
                                  onClick={() => {
                                    setId(mech.id);
                                    setDeleteView(true);
                                  }}
      ...
      {profileView && (
        <div className="profile-view  pt-1">
              ...
              <div className="detail-bar">
                <Row>
                  <Col md={4} className="text-center">
                    ...
                    <button
                      className="btn btn-light btn-sm mt-2 px-3"
                      onClick={() => {
                        setEdit(true);
                        handleEditShow();
                      }}
                  ...
                  <Col md={4} className="">
                    <div className="my-2">
                      {' '}
                      <ImPhone className="mx-3 mb-1" /> {phone}{' '}
                    </div>
                    <div
                  ...
                  <Col md={4} className="">
                    ...
                    <div className="my-2">
                      {' '}
                      <MdLocationOn className="mx-3" /> {address}{' '}
                    </div>
                  </Col>
                </Row>
              </div>
          ...
          >
            <b className="bact-btn">{'<'} Mechanics</b>{' '}
          </span>
      ...
      <Modal
        show={show}
        onHide={profileView ? () => setShow(false) : handleClose}
        size="lg"
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {edit == true ? 'Update Mechanic' : 'Add New Mechanic'}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
              ...
              <Col>
                <Form.Group className="mb-3" controlId="Password">
                  ...
                  {passWarn == true && (
                    <Form.Text style={{ color: 'crimson' }}>
                      {' '}
                      Must be {'(8-20)'} letters, with special &
                      numeric character.{' '}
                    </Form.Text>
                  )}
                </Form.Group>
